...
(w --> ww), and input-specified reduplication (wR --> ww^R). 
...
def get_input_spec_red_pair(w, n, hint="@"):
    '''Returns a pair of an input string and its input-specified reduplication.
    
    **Input-specified reduplication** produces an output string that contains 
    n copies of a given string according to the specified number n.
    
    ::hint:: is an instruction attached to the input sequence to specify the 
    number of time the based should be copied.'''
    return [w + hint * n, w * (n+1)]
...
def main():
    ...
    str_funcs = [get_identity_pair, get_rev_pair, get_total_red_pair]
    str_func_names = ["identity", "rev", "total_red"]
    ...
    train, dev, test, gen = [], [], [], []
    for n in range(1, 4):
        for X, data in zip([train_X, dev_X, test_X, gen_X], 
                           [train, dev, test, gen]):
            
            str_func = lambda X_: get_input_spec_red_pair(X_, n)
            data.extend(list(map(str_func, X)))
    
    folder = join(data_folder, "input_spec_red")
    makedirs(folder, exist_ok=True)
    for data, fname in zip([train, dev, test, gen], 
                           ["train.txt", "dev.txt", "test.txt", "gen.txt"]):
        fp = join(folder, fname)
        save_ds_in_txt(data, fp)
